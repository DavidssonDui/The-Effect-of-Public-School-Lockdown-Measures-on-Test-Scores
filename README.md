...
└── Output/
 ├── figures/ # auto-created by the script (12 figure PDFs)
 └── tables/ # auto-created by the script (4 table .txt files)
```

The `Output/figures/` and `Output/tables/` subfolders are created automatically by the script on first run, so you only need to create `Output/` itself.
